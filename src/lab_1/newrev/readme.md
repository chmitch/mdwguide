...
--REMOVE NOTES IN FUTURE
--Migrate OLAP to Synapse
	-- Data (One-time)
	-- Schema
--Network to accomodate on-premise to Azure
--Migrate SSIS jobs
    --Change target to Synapse to see if break/fix
    --Change target to be ADLS and use Polybase to load into Staging tables (Seems excessive)

Phase 2
--Rebuild ETL to modernize it on MPP Compute (Databricks)
--Copy activity to Lake
--Transform to staging layer in Lake
--Polybase from Lake
--ADF Data flows to Synapse
--Add Data flow for semi-structured data similar to Weblog exampe

Phase 3
-- Real-time OLTP platform thru Kafka Connect (Debezum, Attunity, GoldenGate, etc)
-- Setup environment to replace copy activity in Phase 2
-- Rerun ADF jobs on a more frequent schedule


Phase 4
-- Real-time Clickstream data into Delta Lake
-- Batch Semi-structured clickstream data into Delta Lake
-- Notebook to query data for interactive query capabilitiy


Phase 5
-- Stream it into Warehouse or elsewhere for lambda architecture